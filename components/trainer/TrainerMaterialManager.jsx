  ...
  return (
    <div>
      <div className="flex justify-between items-center">
        <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">My Materials</h1>
            <p className="mt-2 text-slate-600 dark:text-slate-400">Upload and manage your training content.</p>
        </div>
      ...
      <div className="mt-8 grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {materials.map(material => (
          <div key={material.id} className="bg-white dark:bg-slate-800/50 p-5 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col justify-between transition-shadow hover:shadow-md">
            ...
            <div className="mt-4">
                 <button onClick={() => handleViewMaterial(material)} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-center text-violet-700 bg-violet-100 rounded-lg hover:bg-violet-200 dark:bg-violet-500/20 dark:text-violet-300 dark:hover:bg-violet-500/30">
